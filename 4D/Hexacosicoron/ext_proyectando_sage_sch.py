...
n = normal_4d / normal_norm  #normal_4d with length 1
...
Q, _ = np.linalg.qr(np.vstack([n, np.eye(4)]).T)  # 4×4 orthogonal matrix
    #stacks 'n' on top of the identity matrix (5x4 Matrix) and then Transposes it (4x5 Matrix)
    # ===> performs QR decomposition [[[ matrix A (4×5) |---> orthogonal {orthonormal by using n} Q (4×4) and upper-triangular R (4×5) ]]]
    # Takes only the "Q" Matrix [orthonormal matrix whose first column is aligned with n (normalized), and the other columns form an orthonormal completion] and disposes de "R" (_)

basis_3d = Q[:, 1:]  # last 3 columns: orthonormal basis of hyperplane orthogonal to "n" [4×3 matrix]

# Project each 4D point to 3D coordinates in this basis
vertices_3d = []
for x in projected_4d: #points in R^4 lying on the hyperplane (after projection).
    rel = x - x0  # vector in hyperplane direction (parallel to it)
    coords = basis_3d.T @ rel  # 3D coordinates [(3x4) * (4x1) === (3x1) in R^3]
...
with open('hexacosicoron_sage.json', 'w') as f:
